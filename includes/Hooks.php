...
(window.RLQ = window.RLQ || []).push( function () {
    ...
    tabLink.addEventListener( 'click', function ( event ) {
        ...
        mw.loader.using( 'oojs-ui-windows' ).done( function () {
            ...
            var feedbackMaxLength = 1000;
            var windowManager = OO.ui.getWindowManager();
            var textInput = new OO.ui.MultilineTextInputWidget( { rows: 4, maxLength: feedbackMaxLength } );
            var textField = new OO.ui.FieldLayout( textInput, {
                align: 'top',
                label: message
            } );
            ...
            var counter = $( '<div>' ).css( {
                ...
                textAlign: 'right',
                marginTop: '0.2em'
            } ).text( '0 / ' + feedbackMaxLength );
            textInput.\$element.after( counter );

            textInput.on( 'change', function ( value ) {
                var length = value.length;
                counter.text( length + ' / ' + feedbackMaxLength );
                counter.css( 'color', length >= feedbackMaxLength ?
                    '#d33' : ( length >= feedbackMaxLength * 0.9 ? '#ac6600' : '#72777d' ) );
            } );

            var dialogInstance = windowManager.openWindow( 'message', {
                message: textField.\$element
            } );
            dialogInstance.opened.then( function () {
                textInput.focus();
            } );

            var promptPromise = dialogInstance.closed.then( function ( data ) {
                return data && data.action === 'accept' ? textInput.getValue() : null;
            } );

            promptPromise.done( function ( text ) {
                if ( !text ) {
                    return;
                }
